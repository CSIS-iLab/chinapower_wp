...
if ( ! function_exists( 'chinapower_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function chinapower_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && chinapower_categorized_blog() ) {
			...
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'chinapower' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'chinapower' ) );
		if ( $tags_list ) {
			/* translators: 1: list of tags. */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'chinapower' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'chinapower' ),
					array(
						'span' => array(
							'class' => array(),
						),
	...
	edit_post_link(
		sprintf(
			wp_kses(
				...
				__( 'Edit <span class="screen-reader-text">%s</span>', 'chinapower' ),
				array(
					'span' => array(
						'class' => array(),
					),
